...
#[derive(Debug, Deserialize)]
pub struct ArtifactGeographyFeatures {
    contract_path: PathBuf,
    grid_path: PathBuf,
}

#[derive(Debug, Deserialize)]
pub struct GeographyFeatureContract {
    columns: Vec<String>,
    mean: Vec<f32>,
    std: Vec<f32>,
    #[serde(default)]
...
#[derive(Debug, Clone)]
pub struct GeographyFeatureGrid {
    columns: Vec<String>,
    mean: Vec<f32>,
    std: Vec<f32>,
    clip_abs: Option<f32>,
...
#[derive(Debug, Clone)]
pub struct GeographyFeatureRow {
    latitude: f64,
    longitude: f64,
    values: Vec<f32>,
}
...
impl SourceModelRuntime {
    ...
    pub fn prepare_estimate(&self, request: &EstimateRequest) -> Result<PreparedEstimate> {
        self.prepare_estimate_arrays(request, &[request.single_array()])
    }

    pub fn prepare_estimate_arrays(
        &self,
        request: &EstimateRequest,
        arrays: &[EstimateArray],
    ) -> Result<PreparedEstimate> {
        ...
        let features_shape = [TEMPORAL_BINS, self.input_features];
        let feature_values = features.into_raw_vec_and_offset().0;
        ...
        for source in &self.sources {
            ...
            jobs.push(SourceInferenceJob {
                source_id: source.source_id.clone(),
                label: source.label.clone(),
                onnx_path: source.onnx_path.clone(),
                input_name: source.input_name.clone(),
                output_name: source.output_name.clone(),
                features_shape,
                features: feature_values.clone(),
            });
        }
    ...
    pub fn finish_estimate(
        ...
        source_outputs: &[SourceInferenceOutput],
        ...
        for job in &prepared.jobs {
            let source = self
                .sources
                .iter()
                .find(|source| source.source_id == job.source_id)
                .ok_or_else(|| anyhow!("unknown prepared source {}", job.source_id))?;
            let output = source_outputs
                .iter()
                .find(|output| output.source_id == job.source_id)
                .ok_or_else(|| anyhow!("missing output for source {}", job.source_id))?;
            let climate = source.denormalize_climate(&output.normalized_targets)?;
            let pv = estimate_pv_from_climate(&climate, &prepared.request, &prepared.arrays);
            source_profiles.push(estimate_hourly_pv_from_climate(
                &climate,
                &prepared.request,
                &prepared.arrays,
            ));
        ...
        let ensemble = AnnualPvEnsembleEstimate::from_source_estimates_with_uncertainty(
            source_estimates,
            prepared.uncertainty_multiplier,
        )
        .ok_or_else(|| {
            anyhow!(
                "no applicable source models for {},{}",
                prepared.request.latitude,
                prepared.request.longitude
            )
        })?;

        let estimate = SourceEnsembleEstimateDocument {
            schema_version: 1,
            location: EstimateLocation {
                location_id: prepared.request.location_id.clone(),
                name: prepared.request.name.clone(),
                region: prepared.request.region.clone(),
                latitude: prepared.request.latitude,
                longitude: prepared.request.longitude,
            },
            system: EstimateSystem {
                peak_power_kwp: total_peak_power_kwp(&prepared.arrays),
                loss_pct: prepared.request.loss_pct,
                tilt_deg: weighted_array_value(&prepared.arrays, |array| array.tilt_deg),
                aspect_deg: weighted_array_value(&prepared.arrays, |array| array.azimuth_deg),
                storage_usable_kwh: prepared.request.storage_usable_kwh,
            },
            coverage: EstimateCoverage {
                pvgis_sarah3_applicable: prepared.pvgis_sarah3_applicable,
                applicable_sources: prepared.applicable_sources.clone(),
            },
            ensemble_estimate: ensemble,
            references: json!({
                "model_family": prepared.model_family,
                "input_features": prepared.input_features,
                "temporal_bins": prepared.temporal_bins,
                "target_names": prepared.target_names,
                "arrays": prepared.arrays.iter().map(|array| {
        ...
        let production_profile =
            production_profile_from_sources(&estimate.ensemble_estimate, &source_profiles)?;
        Ok(FinishedEstimate {
            estimate,
            production_profile,
        })
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct PreparedEstimate {
    pub request: EstimateRequest,
    pub arrays: Vec<EstimateArray>,
    pub jobs: Vec<SourceInferenceJob>,
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct SourceInferenceJob {
    pub source_id: String,
    pub label: String,
    pub onnx_path: PathBuf,
    pub input_name: String,
    pub output_name: String,
    pub features_shape: [usize; 2],
    pub features: Vec<f32>,
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct SourceInferenceOutput {
    pub source_id: String,
    pub normalized_targets: Vec<f32>,
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct FinishedEstimate {
    pub estimate: SourceEnsembleEstimateDocument,
...
fn load_sources(model_dir: &Path, sources: Vec<ArtifactSource>) -> Result<Vec<RuntimeSource>> {
    sources
        .into_iter()
...
fn load_embedded_sources(sources: Vec<ArtifactSource>) -> Result<Vec<RuntimeSource>> {
    sources
        .into_iter()
        .filter(|source| source.active)
...
fn load_source(model_dir: &Path, source: ArtifactSource) -> Result<RuntimeSource> {
    let onnx_path = model_dir.join(&source.onnx_path);
    source_runtime(onnx_path, model_dir, source)
}

fn load_embedded_source(source: ArtifactSource) -> Result<RuntimeSource> {
    source_runtime(source.onnx_path.clone(), Path::new(""), source)
}
...
impl RuntimeSource {
    ...
    pub fn denormalize_climate(
        &self,
        normalized_targets: &[f32],
    ) -> Result<[[f64; TARGETS]; TEMPORAL_BINS]> {
        if normalized_targets.len() != TEMPORAL_BINS * TARGETS {
            bail!(
        ...
        for row in 0..TEMPORAL_BINS {
            for target in 0..TARGETS {
                let normalized = normalized_targets[row * TARGETS + target];
